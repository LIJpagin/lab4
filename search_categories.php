...
<?php include_once __DIR__ . '/components/head.php';
  ...
  $result2 = $connect->query(
    "SELECT id FROM categories 
     WHERE id NOT IN
     (SELECT id_category FROM readers_categories WHERE id_reader = '".$reader_id."')");
...
<body class="search_category_body">
  <?php include_once __DIR__ . '/components/header.php' ?>
  <div style="margin-top: 70px;">
    <h4 style="text-align: center; margin-bottom: 15px;">Поиск новых категорий</h4>
    ...
    <?php if (count($categories_id) == 0) : ?>
      <h6 style="text-align: center; margin-bottom: 15px;">Нет новых категорий</h6>
      <form style="margin-bottom: 0px;">
          <button class="search_button" formaction="subscribe_categories.php">Перейти к подпискам</button>
      </form>
    <?php endif; ?>
  </div>
</body>
